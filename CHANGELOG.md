...
- Added native Matter support, allowing compatible sensors to be used directly by the plugin without requiring Home Assistant.
- Added independent Home Assistant and Matter source selection, allowing the plugin to use Home Assistant only, Matter only, or both sources simultaneously.
- Added Matter sensor commissioning directly from the plugin administration interface using a Matter sharing code generated by Apple Home.
- Integrated Matter devices into the existing device catalog and HomeKit virtual thermostat workflow alongside Home Assistant devices.
- Added Matter device metadata, state synchronization and subscription management for temperature, humidity and battery information.
- Improved Matter device identification and naming, including preservation of custom device names across rediscovery and synchronization.
- Improved Matter commissioning feedback and device management in the administration interface.
- Updated French, English, German and Spanish translations for the new Matter and source-selection features.
- Completely updated the V2 user and developer documentation to cover Home Assistant, Matter and mixed-source configurations.
...
- Ajout de la prise en charge native de Matter, permettant d'utiliser directement des capteurs compatibles dans le plugin sans passer par Home Assistant.
- Ajout de la sélection indépendante des sources Home Assistant et Matter, permettant d'utiliser Home Assistant seul, Matter seul ou les deux sources simultanément.
- Ajout de l'intégration de capteurs Matter directement depuis l'interface d'administration du plugin à l'aide d'un code de partage Matter généré par Apple Maison.
- Intégration des appareils Matter dans le catalogue existant et dans la génération des thermostats virtuels HomeKit, parallèlement aux appareils provenant de Home Assistant.
- Ajout de la gestion des métadonnées, de la synchronisation des états et des abonnements Matter pour la température, l'humidité et la batterie.
- Amélioration de l'identification et du nommage des appareils Matter, avec notamment la conservation des noms personnalisés lors des redécouvertes et synchronisations.
- Amélioration du processus d'ajout Matter et des informations présentées dans l'interface d'administration.
- Mise à jour des traductions française, anglaise, allemande et espagnole pour les nouvelles fonctions Matter et de sélection des sources.
- Mise à jour complète de la documentation utilisateur et développeur V2 pour couvrir les configurations Home Assistant, Matter et l'utilisation simultanée des deux sources.
